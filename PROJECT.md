...
| Workspace mail relay | `workspace-mail-relay/` | Planned authenticated HTTPS gateway on the existing droplet, forwarding approved single-recipient mail through Google Workspace SMTP relay for Timed Quiz and selected WordPress bulk-mail tools; source only, not deployed |
