...
Do not deploy or switch the WordPress endpoint until DNS, TLS, an Internal
Gmail API OAuth client authorized as `info@` for only `gmail.send`, two independent caller secrets, and authorized test-message
